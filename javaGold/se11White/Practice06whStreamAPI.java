/**
 * @title javaGold / se11White / Practice06whStreamAPI.java
 ...
 */
/* Appendix 2021-05-14
 * @costTime 14:11 - 14:46 ( 35 分)
 * @correctRate 正答率 89.47 ％ ( 〇17問 / 全19問 )
 */
package javaGold.se11White;

import javaGold.PracticeEditor;

public class Practice06whStreamAPI {
...
Ｘ （17） B -> A
    ...
    => Optional<T> Stream.min(c)/max(c) 引数 cは Comparator<T>
        ...
        値を取り出すには Optional.get(), orElse(T)が必要
...
〇 （7） C
Ｘ （8） D -> B
    => list.stream()からストリームを新たに生成しているので、
    *  １つのStreamで終端処理を２回しているわけではない。

    => 別のStreamの結果を c.setRank()でフィールドに保存しているので、
    *  他のStreamでも、その値を参照できる。

    => Stream.filter(Predicate<T>)は falseになった要素を破棄するが、
    *  要素そのものを ture/falseに変更するわけではない。
    *  filter()の破棄の結果はこのStreamだけで、データソースの listを変更するわけではない。

〇 （9） B
〇 （10） D
〇 （11） C
    => 終端処理がないので非表示。
    -> forEach(System.out::print);を加えると表示される。
    *  「Hello : Java Hello : Java Hello : Duke Hello : Duke ...」
    *
    => peek(),forEach()の分が表示されるが、実行順に注意。
    *  メソッドごとに Streamオブジェクトの要素がすべて処理されるのではなく、
    *  要素ごとに終端処理まで通して処理され、次の要素の処理に行く。
...
Ｘ （14） D -> B
    => List<String> Files.readAllLines()
       ...
       Stream<String> BufferdReader.lines()
